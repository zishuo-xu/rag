...
当前项目采用“本地 FastAPI 单体应用 + Redis 队列 Worker + 云端 PostgreSQL/Redis + 本地静态页面”的实现方式。
...
flowchart TD
    ...
    S --> TQ["Redis 文档任务队列"]
    W["Document Worker"] --> TQ
    W --> I["入库服务 IngestService"]
    I --> F["文件解析 / 清洗 / 切分"]
    I --> E["Embedding Provider"]
    A --> Q["问答服务 QAService"]
    Q --> V["pgvector 向量相似度检索"]
    Q --> FT["PostgreSQL 全文检索"]
    Q --> RR["RerankService"]
    Q --> K["关键词检索(兜底/混合)"]
...
13. chunk 向量生成与落库；
14. 基于向量相似度、PostgreSQL 全文检索和关键词的混合检索；
15. 问答历史记录落库；
16. 页面端可直接提问并查看命中片段；
...
36. `document_chunk` 显式落库 `page_start / page_end / semantic_tags_json` 等结构化检索字段。
37. `document_task` 独立记录每次上传 / 重处理任务的状态、阶段、耗时与失败原因。
38. 首页提供“RAG 分享体验区”，支持直接输入短资料和问题，演示一次不落库的迷你 RAG 过程。
...
18. 点击文档“详情”后，可在页面查看最近任务记录和任务耗时。
19. 首页提供“RAG 过程可视化”流程区，可用于分享文档入库链路和问答链路。
20. 首页提供“RAG 分享体验区”，可直接输入短资料和问题，演示清洗、切分、召回和生成过程。
21. 分享体验区按阶段依次展示清洗结果、切片结果、向量结果、召回证据和最终答案，而不是一次性返回全部内容。
22. 分享体验区提供步骤高亮和当前阶段指示，更适合现场分享讲解。
...
说明：当前问答链路已经接入，检索默认采用“Query Rewrite -> pgvector 向量召回 + PostgreSQL 全文召回 -> 关键词特征补充 -> rerank -> 去重合并”的流程；如果配置了 `LLM_API_KEY` 和 `LLM_MODEL`，会在检索结果基础上调用 OpenAI 兼容接口生成最终答案。对 DeepSeek 这类 provider，当前会优先直接走 `Chat Completions`。问答处理阶段会写入 Redis，前端通过轮询进度接口做实时展示；历史详情接口会返回单次问答的答案、引用、模型、耗时以及 LLM 输入/输出。`POST /api/qa/demo` 则提供一条不落库的迷你 RAG 演示链路，适合分享时直接展示“清洗、切分、召回、生成”的过程。
...
4. 文档解析与切分；
5. chunk 向量化；
6. pgvector 检索、PostgreSQL 全文检索与混合排序；
7. 可选的大模型答案生成；
8. 本地规则型 rerank；
...
1. 上传和重处理先快速入队；
2. 独立 worker 从 Redis 队列消费任务并完成解析、切分和向量化；
3. 前端通过文档列表轮询状态；
4. 文档记录会同步写入 `processing_stage / processing_message` 供前端展示；
5. 文档记录会保存 `processing_duration_ms / stage_durations_json`，用于展示总耗时和阶段耗时；
6. Web 服务和文档处理已解耦，服务重启不会直接中断已入队任务。
7. 健康检查会展示 worker 心跳状态，worker 启动时会自动恢复长时间陈旧的 `PROCESSING` 任务。
8. PDF 解析会保留页级标记，并在无文字层时给出更明确的扫描版提示。
...
3. 处理过程中会持续累计 `stage_durations_json`；
4. 解析和切分成功后更新为 `SUCCESS`，阶段写为 `completed`，并记录总耗时；
5. 解析失败后更新为 `FAILED`，阶段写为 `failed`，并记录总耗时和 `error_message`；
6. 删除接口当前直接删除数据库记录和本地文件，不保留 `DELETED` 记录。
...
1. `19` 题真实问法全量端到端回归成本较高，当前更适合先用 `11` 题小样本快速验证生成侧改动方向，再决定是否放大到全量集；
2. 本轮“更密的事实保真提示词”在 `11` 题样本上带来了：
   - `answer_keyword_recall: 83.33% -> 89.39%`
   - `golden_answer_recall: 57.86% -> 60.24%`
   - `ROUGE-L F1: 67.30% -> 72.97%`
   但同时也带来了：
   - `answer_token_f1: 42.12% -> 37.97%`
   - `average_latency: 3012.0ms -> 4448.1ms`
3. 因为这轮收益不够稳定，且时延明显上升，所以当前默认主线不切换到该版本，只保留实验报告用于后续对比。
4. 本轮“句子-引用绑定提示词”在同一批 `11` 题样本上带来了：
   - `answer_keyword_recall: 83.33% -> 86.36%`
   - `ROUGE-L F1: 67.30% -> 68.84%`
   但同时：
   - `golden_answer_recall: 57.86% -> 57.86%`
   - `answer_token_f1: 42.12% -> 39.62%`
   - `average_latency: 3012.0ms -> 4418.4ms`
5. 因为该实验没有提升标准答案覆盖率，且显著拉高时延，所以同样不进入默认主线，只保留实验报告用于后续比较。
6. 本轮“题型驱动的上下文组织”在同一批 `11` 题样本上带来了：
   - `answer_keyword_recall: 83.33% -> 92.42%`
   - `golden_answer_recall: 57.86% -> 63.57%`
   - `ROUGE-L F1: 67.30% -> 71.66%`
   - `answer_token_f1: 42.12% -> 42.06%`
   - `section_precision_at_4: 26.52% -> 30.30%`
   同时带来了：
   - `average_latency: 3012.0ms -> 4575.6ms`
7. 因为这轮在标准答案覆盖率和整体表达相似度上都有净增益，且 `answer_token_f1` 基本持平，所以当前默认生成策略保留这版“题型驱动的上下文组织”，并接受其带来的额外时延成本。
8. 本轮“按题型控制输出长度上限”在同一批 `11` 题样本上出现了整体回落：
   - `answer_keyword_recall: 92.42% -> 80.30%`
   - `golden_answer_recall: 63.57% -> 53.57%`
   - `answer_token_f1: 42.06% -> 35.44%`
   - `ROUGE-L F1: 71.66% -> 68.68%`
   - `average_latency: 4575.6ms -> 4449.3ms`
9. 因为这轮只是小幅降低时延，却明显损失答案质量，所以不进入默认主线，只保留实验报告用于后续比较。
