...
12. 问答历史详情回放组装；
13. LLM 输入与输出持久化；
14. 检索结果去重与相邻 chunk 合并；
15. Query Rewrite 问题改写与检索增强；
16. RAG 评测执行与指标汇总；
17. 章节标题提取与 `section_title` 落库；
18. 语义标签提取与语义标签加分；
19. fallback 答案组织与依据要点生成；
20. 业务异常转换。
...
sequenceDiagram
    ...
    API->>DS: save_upload()
    DS->>FS: 保存原始文件
    DS->>DB: 新建document(PROCESSING)
    DS->>IS: process_document()
...
1. 上传成功后先写入 `PROCESSING`；
2. 解析和切分成功后更新为 `SUCCESS`；
3. 解析失败后更新为 `FAILED` 并记录 `error_message`；
4. 删除接口当前直接删除数据库记录和本地文件，不保留 `DELETED` 记录。
...
8. 老数据不会自动重新处理；
9. 当前 rerank 为本地规则型实现，尚未接入外部高质量 rerank 模型；
